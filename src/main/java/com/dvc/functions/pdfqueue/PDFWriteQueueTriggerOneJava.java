 ...
 */
public class PDFWriteQueueTriggerOneJava {
        /**
         ...
         */
        @FunctionName("PDFWriteQueueTriggerOneJava")
        public void run(@QueueTrigger(name = "messageOne", queueName = "pdf-queue-one", connection = "AzureStorageConnectionString") String message,
                ...
                rList = new RecipientsDownloadDao().getRecipentsListByBatch(pdfMessage.getBatchNo(), 0,
                                pdfMessage.getOffset(), pdfMessage.getPartnerSyskey(), pdfMessage.isAdmin(), false);

                if (rList.size() > PDFUtil.INDEX_ZERO) {
                        List<String> existedPDFList = new ArrayList<>();
                        BlobServiceClient blobServiceClient = new BlobServiceClientBuilder()
                                        .connectionString(System.getenv(PDFUtil.BLOB_CONNECTION_ENV_NAME))
                                        .buildClient();
                        ...
                        new PDFUtil().writeFileAtBlobStorage(
                                        new PDFUtil().generateFolderNameForBatch(rList) + PDFUtil.EXCEL_EXTENSION,
                                        excelArr, folderName, PDFUtil.EXCEL_CONTENT_TYPE, containerClient);

                        new PDFUtil().checkAndWritePDF(folderName, rList, pdfMessage.isOverride(), existedPDFList,
                                        containerClient);

                        existedPDFList = new ArrayList<>();
                        existedPDFList = new PDFUtil().getExistedPDFNameList(containerClient, blobFolderName);
                        new RecipientsDownloadDao().updateRecordStatusBatchUpload(pdfMessage.getBatchNo(),
                                        existedPDFList.size(), folderName, context);
                }
        }
}
