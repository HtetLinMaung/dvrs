 ...
 */
public class GetBatchDetails {
    /**
     * This function listens at endpoint "/api/GetBatchDetails". Two ways to invoke
     * it using "curl" command in bash: 1. curl -d "HTTP Body" {your
     * host}/api/GetBatchDetails 2. curl {your
     * host}/api/GetBatchDetails?name=HTTP%20Query
     */
    ...
    public HttpResponseMessage run(@HttpTrigger(name = "req", methods = {
        ...
        try {
            ...
            if (auth.getTokenData().getRole().equals("Partner")
                    && !dao.isOwnBatch(dto.getBatchuploadsyskey(), auth.getTokenData().getPartnersyskey())) {
            ...
            PaginationResponse<Map<String, Object>> resData = new BatchUploadDao().getBatchDetails(dto);
