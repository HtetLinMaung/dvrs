 ...
 */
public class GetPiUsagesOnCenter {
    /**
     * This function listens at endpoint "/api/GetPiUsagesOnCenterByPartner". Two
     * ways to invoke it using "curl" command in bash: 1. curl -d "HTTP Body" {your
     * host}/api/GetPiUsagesOnCenterByPartner 2. curl {your
     * host}/api/GetPiUsagesOnCenterByPartner?name=HTTP%20Query
     */
    @FunctionName("getpiusagesoncenter")
    public HttpResponseMessage run(@HttpTrigger(name = "req", methods = {
        ...
        try {
            ...
            if (request.getQueryParameters().get("partnersyskey") == null) {
                ...
                String partnersyskey = request.getQueryParameters().get("partnersyskey");
                res.put("datalist", dao.getPiUsagesOnCenterByPartner(partnersyskey));
            }
